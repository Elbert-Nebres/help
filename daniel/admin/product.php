    ...
    min-height: 200px;
      ...
      <script type="text/javascript" src="https://cdn.jsdelivr.net/jsbarcode/3.5.1/JsBarcode.all.min.js"></script>
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Check if quantity is less than or equal to 5
            if ($quantity <= 5) {
                echo '<div class="product-low-stock">';
                echo '<p>Item: ' . htmlspecialchars($row['item']) . '</p>';
                echo '<p>Category: ' . htmlspecialchars($row['category']) . '</p>';
                echo '<p>Quantity: ' . htmlspecialchars($row['quantity']) . '</p>';
                echo '<p>Price: ₱ ' . number_format($row['price'], 2) . '</p>';
                echo '<p>Description: ' . htmlspecialchars(substr($row['description'], 0, 100)) . '...</p>';
            } else {
                echo '<div class="product-in-stock">';
                echo '<p>Item: ' . htmlspecialchars($row['item']) . '</p>';
                echo '<p>Category: ' . htmlspecialchars($row['category']) . '</p>';
                echo '<p>Quantity: ' . htmlspecialchars($row['quantity']) . '</p>';
                echo '<p>Price: ₱ ' . number_format($row['price'], 2) . '</p>';
            ...
            // Add Update Product button for each product
            echo '<input type="button" value="Update Product" class="btn btn-primary" 
                   onclick="updateProduct(' . htmlspecialchars($row['id'], ENT_QUOTES) . ')">';
            echo '</div>';
        }
        ?>
        ...
        function updateProduct(id) {
            if (id) {
                window.location.href = 'update_product.php?id=' + encodeURIComponent(id);
            }
        }
        </script>
